    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    if match == false then
                                -- this makes the block return if the selected block doesnt match with other block
						Timer.tween(0.1, {      -- first it will tween, creating an animation or movement of block
							[self.highlightedTile] = {x = newTile.x, y = newTile.y},
							[newTile] = {x = self.highlightedTile.x, y = self.highlightedTile.y}
						})
						tempX = self.highlightedTile.gridX
						tempY = self.highlightedTile.gridY         -- basically the x and y will be stored temporarily
                                                                   -- then grabs the new coordinates and store it.
						self.highlightedTile.gridX = newTile.gridX  
						self.highlightedTile.gridY = newTile.gridY
						...
						self.board.tiles[self.highlightedTile.gridY][self.highlightedTile.gridX] =
							self.highlightedTile
						self.board.tiles[newTile.gridY][newTile.gridX] = newTile       -- then the original coordinates
                                                                                       -- will just return to its self
						gSounds['error']:stop()
						gSounds['error']:play()

						self.highlightedTile = nil
					else
						self.matches = match            -- if it matches, then it will be true
...
-----------------------------------------------------------
        ...
        for k, match in pairs(matches) do
            ...
            for i, tile in pairs(match) do
                ...
                self.timer = self.timer + 1             -- plus 1 para fair
            end
        end
-----------------------------------------------------------
...
function PlayState:render()
    ...
    if self.paused then
        ...
        love.graphics.rectangle('fill', VIRTUAL_WIDTH / 2 - 136, VIRTUAL_HEIGHT / 2 - 68, 270, 128, 20)
        
        love.graphics.setColor(99, 155, 255, 255)
        love.graphics.setFont(gFonts['large'])
        love.graphics.printf("PAUSED", 0, VIRTUAL_HEIGHT / 2 - 8, VIRTUAL_WIDTH, 'center')
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('Press SPACE to Continue', 0, 100, VIRTUAL_WIDTH, 'center')
    else
    -- render board of tiles
        ...
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('Level: ' .. tostring(self.level), 20, 40, 182, 'center')
        love.graphics.printf('Score: ' .. tostring(self.score), 20, 60, 182, 'center')
        love.graphics.printf('Goal : ' .. tostring(self.scoreGoal), 20, 80, 182, 'center')
        love.graphics.printf('Timer: ' .. tostring(self.timer), 20, 100, 182, 'center')
    -----------------------------------------------------------
        love.graphics.setFont(gFonts['small'])
        love.graphics.printf('Press SPACE to pause', 20, 20, 182, 'center')
